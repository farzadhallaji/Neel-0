...
import com.neelnetwork.api.http.alias.{CreateAliasV1Request, CreateAliasV2Request}
import com.neelnetwork.api.http.assets.SponsorFeeRequest._
import com.neelnetwork.api.http.assets._
import com.neelnetwork.api.http.leasing._
import com.neelnetwork.http.BroadcastRoute
import com.neelnetwork.settings.{FunctionalitySettings, RestAPISettings}
import com.neelnetwork.state.diffs.CommonValidation
import com.neelnetwork.state.{Blockchain, ByteStr}
        ...
        json[JsObject] { jsv =>
          ...
          createTransaction(senderPk, enrichedJsv) { tx =>
            CommonValidation.getMinFee(blockchain, functionalitySettings, blockchain.height, tx).map {
              case (assetId, assetAmount, neelAmount) =>
                Json.obj(
                  "feeAssetId" -> assetId,
  ...
  private def txToExtendedJson(tx: Transaction): JsObject = {
    ...
    tx match {
      case lease: LeaseTransaction =>
        import com.neelnetwork.transaction.lease.LeaseTransaction.Status._
