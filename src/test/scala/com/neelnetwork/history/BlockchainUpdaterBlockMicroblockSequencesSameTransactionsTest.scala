...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
      ...
      case (gen, rest) =>
        val finalMinerBalances = rest.map {
          case (a @ (bmb: BlockAndMicroblockSequence, last: Block)) =>
            withDomain(MicroblocksActivatedAt0NeelSettings) { d =>
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(PrivateKeyAccount, GenesisTransaction, TransferTransactionV1, Int)] = for {
      ...
      payment: TransferTransactionV1 = createNeelTransfer(master, master, amt, fee, ts).explicitGet()
    } yield (miner, genesis, payment, ts)
    scenario(preconditionsAndPayments, MicroblocksActivatedAt0NeelSettings) {
  ...
  def randomPayment(accs: Seq[PrivateKeyAccount], ts: Long): Gen[TransferTransactionV1] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createNeelTransfer(from, to, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[PrivateKeyAccount], PrivateKeyAccount, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice, TOTAL_NEEL / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob, TOTAL_NEEL / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie, TOTAL_NEEL / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave, TOTAL_NEEL / 4, ts + 4).explicitGet()
    } yield
