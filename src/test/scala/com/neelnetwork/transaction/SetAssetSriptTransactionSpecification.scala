...
import com.neelnetwork.account.{AddressScheme, PublicKeyAccount}
...
class SetAssetScriptTransactionSpecification extends GenericTransactionSpecification[SetAssetScriptTransaction] {
  def transactionParser: com.neelnetwork.transaction.TransactionParserFor[SetAssetScriptTransaction] = SetAssetScriptTransaction
  def updateProofs(tx: SetAssetScriptTransaction, p: Proofs): SetAssetScriptTransaction = {
    tx.copy(proofs = p)
  }
  def generator: Gen[((Seq[com.neelnetwork.transaction.Transaction], SetAssetScriptTransaction))] = setAssetScriptTransactionGen
