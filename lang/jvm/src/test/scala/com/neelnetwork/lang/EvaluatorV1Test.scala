...
import com.neelnetwork.lang.v1.evaluator.ctx.impl.{CryptoContext, EnvironmentFunctions, PureContext, _}
...
import com.neelnetwork.utils.{Base58, Base64}
...
class EvaluatorV1Test extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  private def defaultFullContext(environment: Environment): CTX = Monoid.combineAll(
    Seq(
      ...
      pureContext,
      NeelContext.build(V1, environment, isTokenContext = false)
    )
  )
