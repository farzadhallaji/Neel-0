package com.neelnetwork.state.diffs.smart
...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithPureFunctions(tx, t))}
       |${dropLastLine(scriptWithNeelFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && neel else crypto""".stripMargin
  ...
  def scriptWithNeelFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Neel context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && neelBalance(tx.sender) != 0
       |
       | let neel = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | neel""".stripMargin
