...
import com.neelnetwork.account.PublicKeyAccount._
import com.neelnetwork.account._
import com.neelnetwork.lang.Global
...
import com.neelnetwork.lang.v1.testing.ScriptGen
import com.neelnetwork.settings.Constants
import com.neelnetwork.state._
import com.neelnetwork.state.diffs.ENOUGH_AMT
...
trait TransactionGenBase extends ScriptGen with NTPTime { _: Suite =>
  ...
  protected def neel(n: Float): Long = (n * 100000000L).toLong
  ...
  val neelAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, neelAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))
  ...
  def neelTransferGeneratorP(sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, None, None)

  def neelTransferGeneratorP(timestamp: Long, sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
  ...
  def createNeelTransfer(sender: PrivateKeyAccount,
                          recipient: Address,
                          amount: Long,
                          fee: Long,
  ...
  val transferWithNeelFeeGen = for {
  ...
  val selfTransferWithNeelFeeGen: Gen[TransferTransactionV1] = for {
