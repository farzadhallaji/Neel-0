package com.neelnetwork.database

import cats.kernel.Monoid
import com.google.common.cache.CacheBuilder
import com.neelnetwork.account.{Address, Alias}
import com.neelnetwork.block.{Block, BlockHeader}
import com.neelnetwork.database.patch.DisableHijackedAliases
...
import com.neelnetwork.state.reader.LeaseDetails
import com.neelnetwork.transaction.Transaction.Type
import com.neelnetwork.transaction.ValidationError.{AliasDoesNotExist, AliasIsDisabled, GenericError}
import com.neelnetwork.transaction._
import com.neelnetwork.transaction.assets._
import com.neelnetwork.transaction.assets.exchange.ExchangeTransaction
import com.neelnetwork.transaction.lease.{LeaseCancelTransaction, LeaseTransaction}
...
class LevelDBWriter(writableDB: DB, fs: FunctionalitySettings, val maxCacheSize: Int, val maxRollbackDepth: Int, val rememberBlocksInterval: Long)
  ...
  override def balance(address: Address, mayBeAssetId: Option[AssetId]): Long = readOnly { db =>
    addressId(address).fold(0L) { addressId =>
      mayBeAssetId match {
        ...
        case None          => db.fromHistory(Keys.neelBalanceHistory(addressId), Keys.neelBalance(addressId)).getOrElse(0L)
      }
    }
  }

  private def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.neelBalanceHistory(addressId), Keys.neelBalance(addressId)).getOrElse(0L),
  ...
  override protected def doAppend(block: Block,
    ...
    val newAddressesForNeel = ArrayBuffer.empty[BigInt]
    val updatedBalanceAddresses = for ((addressId, balance) <- neelBalances) yield {
      val kwbh = Keys.neelBalanceHistory(addressId)
      val wbh  = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForNeel += addressId
      }
      rw.put(Keys.neelBalance(addressId)(height), balance)
      expiredKeys ++= updateHistory(rw, wbh, kwbh, balanceThreshold, Keys.neelBalance(addressId))
      addressId
    }

    val changedAddresses = addressTransactions.keys ++ updatedBalanceAddresses

    if (newAddressesForNeel.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForNeelSeqNr) + 1
      rw.put(Keys.addressesForNeelSeqNr, newSeqNr)
      rw.put(Keys.addressesForNeel(newSeqNr), newAddressesForNeel)
    }
      ...
      val discardedBlocks: Seq[Block] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for (addressId <- rw.get(Keys.changedAddresses(currentHeight))) {
            ...
            rw.delete(Keys.neelBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.neelBalanceHistory(addressId), currentHeight)
  ...
  override def balanceSnapshots(address: Address, from: Int, to: Int): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      val wbh = slice(db.get(Keys.neelBalanceHistory(addressId)), from, to)
      val lbh = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, to)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.neelBalance(addressId)(wh)))
  ...
  override def neelDistribution(height: Int): Map[Address, Long] = readOnly { db =>
    (for {
      seqNr     <- (1 to db.get(Keys.addressesForNeelSeqNr)).par
      addressId <- db.get(Keys.addressesForNeel(seqNr)).par
      history = db.get(Keys.neelBalanceHistory(addressId))
      actualHeight <- history.partition(_ > height)._2.headOption
      balance = db.get(Keys.neelBalance(addressId)(actualHeight))
      if balance > 0
    } yield db.get(Keys.idToAddress(addressId)) -> balance).toMap.seq
  }
}
