...
import com.neelnetwork.block.{Block, BlockHeader, MicroBlock}
import com.neelnetwork.features.BlockchainFeatures
import com.neelnetwork.features.FeatureProvider._
import com.neelnetwork.metrics.{Instrumented, TxsInBlockchainStats}
import com.neelnetwork.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.neelnetwork.settings.NeelSettings
import com.neelnetwork.state.diffs.BlockDiffer
import com.neelnetwork.state.reader.{CompositeBlockchain, LeaseDetails}
import com.neelnetwork.transaction.Transaction.Type
import com.neelnetwork.transaction.ValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
import com.neelnetwork.transaction._
import com.neelnetwork.transaction.lease._
import com.neelnetwork.transaction.smart.script.Script
import com.neelnetwork.utils.{ScorexLogging, Time, UnsupportedFeature, forceStopApplication}
import kamon.Kamon
import kamon.metric.MeasurementUnit
import monix.reactive.Observable
import monix.reactive.subjects.ConcurrentSubject

class BlockchainUpdaterImpl(blockchain: Blockchain, settings: NeelSettings, time: Time)
    extends BlockchainUpdater
    with NG
    with ScorexLogging
    with Instrumented {

  import com.neelnetwork.state.BlockchainUpdaterImpl._
  ...
  /** Retrieves Neel balance snapshot in the [from, to] range (inclusive) */
  ...
  override def neelDistribution(height: Int): Map[Address, Long] = ngState.fold(blockchain.neelDistribution(height)) { ng =>
    val innerDistribution = blockchain.neelDistribution(height)
  ...
  /** Builds a new portfolio map by applying a partial function to all portfolios on which the function is defined.
    *
    * @note Portfolios passed to `pf` only contain Neel and Leasing balances to improve performance */
