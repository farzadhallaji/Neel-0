...
import com.neelnetwork.settings.{Constants, TestFunctionalitySettings}
...
import com.neelnetwork.transaction.assets.{IssueTransactionV1, SponsorFeeTransaction}
import com.neelnetwork.transaction.lease.LeaseTransactionV1
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      neelOverspend = TransferTransactionV1
        .selfSigned(None, master, recipient.toAddress, 1000000, ts + 3, Some(assetId), fee, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, neelOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, neelOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000 NEEL")
        }
        assertDiffEi(setupBlocks, block(Seq(neelOverspend)), s) { blockDiffEi =>
          if (neelOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
            blockDiffEi should produce("negative neel balance")
        }
    }
  }

  property("not enough neel to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000000 NEEL: 99999999")
        }
    }
  }
  ...
  property("sponsor has no NEEL but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      assetTransfer = TransferTransactionV1
        ...
        .right
        .get
      neelTransfer = TransferTransactionV1
        .selfSigned(None, master, recipient, 99800000, ts + 4, None, 100000, Array.emptyByteArray)
        .right
        .get
      backNeelTransfer = TransferTransactionV1
        .selfSigned(None, recipient, master, 100000, ts + 5, Some(assetId), 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, neelTransfer, backNeelTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, neelTransfer, backNeelTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, neelTransfer))), block(Seq(backNeelTransfer)), s) {
          case (diff, state) =>
