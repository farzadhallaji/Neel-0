...
class MatcherMassOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceSecondNeelPair = AssetPair(ByteStr.decodeBase58(aliceSecondAsset).toOption, None)
    ...
    val aliceOrderIdFill = matcherNode
      .placeOrder(aliceAcc, aliceSecondNeelPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 10.minutes)
      .message
      .id
    ...
    val aliceOrderToCancelId =
      matcherNode
        .placeOrder(aliceAcc, aliceSecondNeelPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 70.seconds)
        .message
        .id

    val aliceActiveOrderId = matcherNode
      .placeOrder(aliceAcc, aliceSecondNeelPair, OrderType.SELL, 3, Order.PriceConstant + 100000000, matcherFee, orderVersion, 10.minutes)
      .message
      .id

    matcherNode.waitOrderStatus(aliceSecondNeelPair, aliceOrderToCancelId, "Cancelled", 2.minutes)

    //Bob orders should partially fill one Alice order and fill another
    ordersRequestsGen(2, bobAcc, aliceSecondNeelPair, OrderType.BUY, 2)

    //check orders after filling
    matcherNode.waitOrderStatus(aliceSecondNeelPair, alicePartialOrderId, "PartiallyFilled")

    orderStatus(aliceAcc, aliceSecondNeelPair, aliceOrderIdFill, "Filled")
    orderStatus(aliceAcc, aliceSecondNeelPair, alicePartialOrderId, "PartiallyFilled")
    ...
    "Mass orders creation with random lifetime. Active orders still in list" in {
      ...
      ordersRequestsGen(orderLimit + 1, aliceAcc, aliceNeelPair, OrderType.SELL, 3)

      //wait for some orders cancelled
      Thread.sleep(5000)
      val bobsOrderIds = ordersRequestsGen(orderLimit + 1, bobAcc, aliceNeelPair, OrderType.BUY, 2)
      Thread.sleep(5000)
      ...
      matcherNode.waitOrderStatus(aliceSecondNeelPair, aliceActiveOrderId, "Accepted")
      matcherNode.waitOrderStatus(aliceSecondNeelPair, alicePartialOrderId, "PartiallyFilled")

      matcherNode.fullOrderHistory(bobAcc).map(_.id) should equal(bobsOrderIds.drop(1).reverse)
      matcherNode.orderHistoryByPair(bobAcc, aliceNeelPair).map(_.id) should equal(bobsOrderIds.drop(1).reverse)
    }
    ...
    "check order history orders count after fill" in {
      ...
      val aliceOrderHistoryByPair = matcherNode.orderHistoryByPair(aliceAcc, aliceNeelPair)
