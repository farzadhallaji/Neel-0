...
import com.neelnetwork.it.api.SyncMatcherHttpApi
...
class MatcherTickerTestSuite
  ...
  "matcher ticker validation" - {
    "get tickers for unavailable asset should produce error" in {
      SyncMatcherHttpApi.assertNotFoundAndMessage(matcherNode.marketStatus(wctNeelPair), s"Invalid Asset ID: ${IssueEightDigitAssetTx.id()}")
    }

    "status of empty orderbook" in {
//    TODO: add error message after fix of https://neelnetwork.atlassian.net/browse/NODE-1151
//      SyncMatcherHttpApi.assertNotFoundAndMessage(matcherNode.marketStatus(neelUsdPair), s"")
    }

    "error of non-existed order" in {
      //TODO: add error message after fix of https://neelnetwork.atlassian.net/browse/NODE-1151
//      SyncMatcherHttpApi.assertNotFoundAndMessage(matcherNode.orderStatus(IssueUsdTx.id().toString, neelUsdPair), s"")
    }

    "try to work with incorrect pair" in {
      val usdNeelPair = AssetPair(
      ...
      assert(
        matcherNode
          .matcherGet(s"/matcher/orderbook/${usdNeelPair.amountAssetStr}/${usdNeelPair.priceAssetStr}/status", statusCode = 301)
          .getHeader("Location")
          .contains(s"NEEL/${usdNeelPair.amountAssetStr}"))

      //TODO: add error message after fix of https://neelnetwork.atlassian.net/browse/NODE-1151
//      SyncMatcherHttpApi.assertNotFoundAndMessage(matcherNode.placeOrder(aliceNode, usdNeelPair, OrderType.BUY, 1.neel, 200), "")
    }
    ...
    "place ask order for second pair" in {
      matcherNode.placeOrder(bobNode.privateKey, wctNeelPair, OrderType.SELL, askAmount, askPrice, matcherFee)
      val bobOrder = matcherNode.placeOrder(bobNode.privateKey, wctNeelPair, OrderType.SELL, askAmount, askPrice, matcherFee).message.id
      matcherNode.waitOrderStatus(wctNeelPair, bobOrder, "Accepted")
      val r = matcherNode.marketStatus(wctNeelPair)
      r.lastPrice shouldBe None
      r.lastSide shouldBe None
      r.bid shouldBe None
...
object MatcherTickerTestSuite {
  ...
  private val minerDisabled = parseString("neel.miner.enable = no")
