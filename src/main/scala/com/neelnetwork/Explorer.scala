...
import com.neelnetwork.account.{Address, AddressScheme}
import com.neelnetwork.database.{Keys, LevelDBWriter}
...
import com.neelnetwork.utils.{Base58, Base64, ScorexLogging}
...
object Explorer extends ScorexLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val configFilename = Try(args(0)).toOption.getOrElse("neel-testnet.conf")

    val settings = NeelSettings.fromConfig(loadConfig(ConfigFactory.parseFile(new File(configFilename))))
      ...
      flag match {
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.neelBalance(addressId)(h)
