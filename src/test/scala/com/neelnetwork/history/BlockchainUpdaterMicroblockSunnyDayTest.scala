...
import com.neelnetwork.account.{Address, AddressOrAlias, PrivateKeyAccount}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- neelTransferGeneratorP(master, alice)
    aliceToBob  = createNeelTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createNeelTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0NeelSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0NeelSettings) { db =>
