...
import com.neelnetwork.account.{Address, Alias, PublicKeyAccount}
...
import com.neelnetwork.transaction.{AssetId, Transaction}
...
object Sponsorship {
  ...
  def toNeel(assetFee: Long, sponsorship: Long): Long = {
    val neel = (BigDecimal(assetFee) * BigDecimal(CommonValidation.FeeUnit)) / BigDecimal(sponsorship)
    if (neel > Long.MaxValue) {
      throw new java.lang.ArithmeticException("Overflow")
    }
    neel.toLong
  }

  def fromNeel(neelFee: Long, sponsorship: Long): Long = {
    val assetFee = (BigDecimal(neelFee) / BigDecimal(CommonValidation.FeeUnit)) * BigDecimal(sponsorship)
