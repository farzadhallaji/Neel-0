...
import com.neelnetwork.api.http.assets.SignedTransferV1Request
import com.neelnetwork.crypto
import com.neelnetwork.it.NTPTime
...
import com.neelnetwork.transaction.assets.exchange.{AssetPair, Order, _}
...
class SignAndBroadcastApiSuite extends BaseTransactionSuite with NTPTime {
  test("height should always be reported for transactions") {
    val txId = sender.transfer(firstAddress, secondAddress, 1.neel, fee = 1.neel).id
  ...
  test("/transactions/sign should produce issue/reissue/burn/transfer transactions that are good for /transactions/broadcast") {
    for (v <- supportedVersions) {
      ...
      signBroadcastAndCalcFee(
        Json.obj("type" -> 5, "quantity" -> 200.neel, "assetId" -> issueId, "sender" -> firstAddress, "reissuable" -> false),
        usesProofs = isProof,
        version = v
      )
      ...
      signBroadcastAndCalcFee(Json.obj("type" -> 6, "quantity" -> 100.neel, "assetId" -> issueId, "sender" -> firstAddress),
                              usesProofs = isProof,
                              version = v)
  ...
  test("/transactions/sign should produce mass transfer transaction that is good for /transactions/broadcast") {
    signBroadcastAndCalcFee(
      Json.obj(
        ...
        "version"    -> 1,
        "sender"     -> firstAddress,
        "transfers"  -> Json.toJson(Seq(Transfer(secondAddress, 1.neel), Transfer(thirdAddress, 2.neel))),
  ...
  test("/transactions/sign should produce lease/cancel transactions that are good for /transactions/broadcast") {
    for (v <- supportedVersions) {
      val isProof = Option(v).nonEmpty
      val leaseId =
        signBroadcastAndCalcFee(Json.obj("type" -> 8, "sender" -> firstAddress, "amount" -> 1.neel, "recipient" -> secondAddress),
                                usesProofs = isProof,
                                version = v)
  ...
  test("/transactions/sign/{signerAddress} should sign a transaction by key of signerAddress") {
    ...
    assert(signedRequest.amount == 1.neel)
  ...
  test("/transactions/broadcast should produce ExchangeTransaction with custom asset") {
    ...
    for ((o1ver, o2ver, tver) <- Seq(
      ...
      val buyAmount           = 2
      val sellAmount          = 3
      val assetPair           = AssetPair.createAssetPair("NEEL", issueTx).get
