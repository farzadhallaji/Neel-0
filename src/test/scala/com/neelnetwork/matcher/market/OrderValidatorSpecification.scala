...
import com.neelnetwork.matcher.model.Events.{OrderAdded, OrderCanceled}
import com.neelnetwork.matcher.model._
import com.neelnetwork.matcher.{MatcherSettings, MatcherTestData}
import com.neelnetwork.settings.Constants
import com.neelnetwork.state.diffs.produce
import com.neelnetwork.state.{AssetDescription, Blockchain, ByteStr, LeaseBalance, Portfolio}
...
import com.neelnetwork.utils.randomBytes
import com.neelnetwork.{NoShrink, TestTime, WithDB}
...
class OrderValidatorSpecification
  ...
  private val pairNeelBtc = AssetPair(None, Some(wbtc))
  ...
  "OrderValidator" should {
    "allow buying NEEL for BTC without balance for order fee" in
  ...
  private def newBuyOrder: Order =
    buy(pair = pairNeelBtc, amount = 100 * Constants.UnitsInWave, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInWave).toLong))

  private def newBuyOrder(ts: Long): Order =
    buy(pair = pairNeelBtc,
        amount = 100 * Constants.UnitsInWave,
        price = 0.0022,
        matcherFee = Some((0.003 * Constants.UnitsInWave).toLong),
        ts = Some(ts))
