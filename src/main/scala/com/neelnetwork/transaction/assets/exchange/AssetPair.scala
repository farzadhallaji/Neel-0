package com.neelnetwork.transaction.assets.exchange

import com.neelnetwork.state.ByteStr
import com.neelnetwork.transaction._
import com.neelnetwork.transaction.assets.exchange.Order.assetIdBytes
import com.neelnetwork.transaction.assets.exchange.Validation.booleanOperators
...
object AssetPair {
  ...
  def assetIdStr(aid: Option[AssetId]): String = aid.fold(NeelName)(_.base58)

  def extractAssetId(a: String): Try[Option[AssetId]] = a match {
    case `NeelName` => Success(None)
