...
val versionSource = Def.task {
  ...
  val versionFile      = (sourceManaged in Compile).value / "com" / "neelnetwork" / "Version.scala"
...
inTask(assembly)(
  Seq(
    test := {},
    assemblyJarName := s"neel-all-${version.value}.jar",
...
inConfig(Compile)(
  Seq(
    mainClass := Some("com.neelnetwork.Application"),
...
inConfig(Linux)(
  Seq(
    maintainer := "neelnetwork.com",
    packageSummary := "Neel node",
    packageDescription := "Neel node"
  ))

bashScriptExtraDefines += s"""addJava "-Dneel.directory=/var/lib/${normalizedName.value}""""
...
inConfig(Universal)(
  Seq(
    mappings += (baseDirectory.value / s"neel-${network.value}.conf" -> "doc/neel.conf.sample"),
    mappings := {
...
lazy val lang =
  crossProject(JSPlatform, JVMPlatform)
    ...
    .jvmSettings(
      ...
      homepage := Some(url("https://docs.neelnetwork.com/en/technical-details/neel-contracts-language-description/maven-compiler-package.html")),
      licenses := Seq(("MIT", url("https://github.com/neelnetwork/Neel/blob/master/LICENSE"))),
      organization := "com.neelnetwork",
      organizationName := "Neel Platform",
      organizationHomepage := Some(url("https://neelnetwork.com")),
      scmInfo := Some(ScmInfo(url("https://github.com/neelnetwork/Neel"), "git@github.com:neelnetwork/Neel.git", None)),
      developers := List(Developer("petermz", "Peter Zhelezniakov", "peterz@rambler.ru", url("https://neelnetwork.com"))),
