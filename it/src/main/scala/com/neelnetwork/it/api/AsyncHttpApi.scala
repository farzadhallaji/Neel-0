...
import com.neelnetwork.api.http.alias.CreateAliasV1Request
import com.neelnetwork.api.http.assets._
import com.neelnetwork.api.http.leasing.{LeaseCancelV1Request, LeaseV1Request, SignedLeaseCancelV1Request, SignedLeaseV1Request}
import com.neelnetwork.api.http.{AddressApiRoute, ConnectReq, DataRequest}
import com.neelnetwork.features.api.ActivationStatus
import com.neelnetwork.http.DebugApiRoute._
import com.neelnetwork.http.DebugMessage._
import com.neelnetwork.http.{DebugMessage, RollbackParams, api_key}
import com.neelnetwork.it.Node
import com.neelnetwork.it.util.GlobalTimer.{instance => timer}
import com.neelnetwork.it.util._
import com.neelnetwork.state.{DataEntry, EitherExt2, Portfolio}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/neel/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]

    def lease(sourceAddress: String, recipient: String, amount: Long, fee: Long, version: Byte = 2): Future[Transaction] = {
      version match {
        case 2 => { //TODO: @monroid refactor after https://neelnetwork.atlassian.net/browse/NODE-1222 fix
          signAndBroadcast(
            Json.obj(
              "type"      -> 8,
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateNeel/$height").as[Map[String, Long]]
