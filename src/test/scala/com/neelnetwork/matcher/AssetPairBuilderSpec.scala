...
class AssetPairBuilderSpec extends FreeSpec with Matchers with MockFactory {
  ...
  private val priceAssets = ConfigFactory.parseString(s"""neel.matcher {
  ...
  private val pairs = Table(
    ("amount", "price", "result"),
    (NEEL, WUSD.base58, Right(())),
    (WUSD.base58, NEEL, Left("Pair should be reverse")),
    (WBTC.base58, WEUR.base58, Left("Pair should be reverse")),
    (WEUR.base58, WBTC.base58, Right(())),
    (Asset1.base58, NEEL, Right(())),
    (NEEL, Asset1.base58, Left("Pair should be reverse")),
