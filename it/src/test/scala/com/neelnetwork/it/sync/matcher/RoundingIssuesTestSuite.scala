...
import com.neelnetwork.it.api.LevelResponse
...
import com.neelnetwork.transaction.assets.exchange.OrderType
...
class RoundingIssuesTestSuite extends MatcherSuiteBase {
  ...
  "should correctly fill an order with small amount" in {
    ...
    val counter   = matcherNode.prepareOrder(aliceAcc, neelUsdPair, OrderType.BUY, 3100000000L, 238)
    val counterId = matcherNode.placeOrder(counter).message.id

    val submitted   = matcherNode.prepareOrder(bobAcc, neelUsdPair, OrderType.SELL, 425532L, 235)
    val submittedId = matcherNode.placeOrder(submitted).message.id

    val filledAmount = 420169L
    matcherNode.waitOrderStatusAndAmount(neelUsdPair, submittedId, "Filled", Some(filledAmount), 1.minute)
    matcherNode.waitOrderStatusAndAmount(neelUsdPair, counterId, "PartiallyFilled", Some(filledAmount), 1.minute)

    val tx = matcherNode.waitOrderInBlockchain(counterId).head
    matcherNode.cancelOrder(aliceAcc, neelUsdPair, counterId)
  ...
  "should correctly fill 2 counter orders" in {
    val counter1 = matcherNode.prepareOrder(bobAcc, neelUsdPair, OrderType.SELL, 98333333L, 60L)
    matcherNode.placeOrder(counter1).message.id

    val counter2   = matcherNode.prepareOrder(bobAcc, neelUsdPair, OrderType.SELL, 100000000L, 70L)
    val counter2Id = matcherNode.placeOrder(counter2).message.id

    val submitted   = matcherNode.prepareOrder(aliceAcc, neelUsdPair, OrderType.BUY, 100000000L, 1000L)
    val submittedId = matcherNode.placeOrder(submitted).message.id

    matcherNode.waitOrderStatusAndAmount(neelUsdPair, counter2Id, "PartiallyFilled", Some(2857143L), 1.minute)
    matcherNode.waitOrderStatusAndAmount(neelUsdPair, submittedId, "Filled", Some(99523810L), 1.minute)

    withClue("orderBook check") {
      val ob = matcherNode.orderBook(neelUsdPair)
