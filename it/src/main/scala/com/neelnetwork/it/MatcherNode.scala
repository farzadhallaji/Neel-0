...
trait MatcherNode extends BeforeAndAfterAll with Nodes with ScorexLogging {
  ...
  def initialBalances(): Unit = {
    List(matcherNode, aliceNode, bobNode).indices
      .map { i =>
        nodes(i).transfer(nodes(i).address, addresses(i), 10000.neel, 0.001.neel).id
      }
  ...
  def initialScripts(): Unit = {
    for (i <- List(matcherNode, aliceNode, bobNode).indices) {
      ...
      val setScriptTransaction = SetScriptTransaction
        .selfSigned(SetScriptTransaction.supportedVersions.head, pk, Some(script), 0.01.neel, System.currentTimeMillis())
        .right
        .get
  ...
  def setContract(contractText: Option[String], acc: PrivateKeyAccount): String = {
    ...
    val setScriptTransaction = SetScriptTransaction
      .selfSigned(SetScriptTransaction.supportedVersions.head, acc, script, 0.014.neel, System.currentTimeMillis())
      .right
      .get
