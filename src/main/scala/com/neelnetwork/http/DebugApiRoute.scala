...
import com.neelnetwork.mining.{Miner, MinerDebugInfo}
import com.neelnetwork.network.{LocalScoreChanged, PeerDatabase, PeerInfo, _}
import com.neelnetwork.settings.NeelSettings
import com.neelnetwork.state.diffs.TransactionDiffer
import com.neelnetwork.state.{Blockchain, ByteStr, LeaseBalance, NG, Portfolio}
import com.neelnetwork.transaction.ValidationError.InvalidRequestSignature
import com.neelnetwork.transaction._
import com.neelnetwork.transaction.smart.Verifier
import com.neelnetwork.utils.{Base58, ScorexLogging, Time}
...
@Path("/debug")
@Api(value = "/debug")
case class DebugApiRoute(ws: NeelSettings,
                         time: Time,
  ...
  private lazy val neelConfig: JsObject = Json.obj("neel" -> (fullConfig \ "neel").get)

  override val settings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    blocks ~ state ~ info ~ stateNeel ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ historyInfo ~ configInfo ~ print ~ validate
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.neelnetwork.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  def state: Route = (path("state") & get & withAuth) {
    complete(ng.neelDistribution(ng.height).map { case (a, b) => a.stringRepr -> b })
  }

  @Path("/stateNeel/{height}")
  @ApiOperation(value = "State at block", notes = "Get state at specified height", httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "height", value = "height", required = true, dataType = "integer", paramType = "path")
    ))
  def stateNeel: Route = (path("stateNeel" / IntNumber) & get & withAuth) { height =>
    complete(ng.neelDistribution(height).map { case (a, b) => a.stringRepr -> b })
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.neelnetwork.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean]) & withAuth) { full =>
    complete(if (full) fullConfig else neelConfig)
  }
