package com.neelnetwork.dexgen

import java.util.concurrent.ThreadLocalRandom

import cats.Show
import com.neelnetwork.account.{AddressOrAlias, PrivateKeyAccount, PublicKeyAccount}
import com.neelnetwork.api.http.assets.SignedTransferV1Request
import com.neelnetwork.crypto
import com.neelnetwork.dexgen.Worker._
import com.neelnetwork.dexgen.utils.{ApiRequests, GenOrderType}
import com.neelnetwork.it.api.{MatcherResponse, MatcherStatusResponse, OrderbookHistory, Transaction, UnexpectedStatusCodeException}
import com.neelnetwork.it.util._
import com.neelnetwork.matcher.AssetPairBuilder
import com.neelnetwork.matcher.api.CancelOrderRequest
...
class Worker(workerSettings: Settings,
  ...
  def transfer(sender: PrivateKeyAccount, assetId: Option[AssetId], recipient: PrivateKeyAccount, halfBalance: Boolean)(
    ...
    to(endpoint).balance(sender.address, assetId).flatMap { balance =>
      ...
      val transferAmount = assetId.fold(halfAmount - 0.001.neel)(_ => halfAmount)
      ...
      TransferTransactionV1.selfSigned(assetId,
        ...
        case Right(txRequest) =>
          log.info(
            s"[$tag] ${assetId.fold("Neel")(_.base58)} balance of ${sender.address}: $balance, sending $transferAmount to ${recipient.address}")
