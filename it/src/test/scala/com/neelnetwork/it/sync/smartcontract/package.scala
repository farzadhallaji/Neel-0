...
import com.neelnetwork.transaction.assets.exchange.{AssetPair, ExchangeTransactionV2, Order}
...
package object smartcontract {
  ...
  def neelContextScript(dtx: DataTransaction, accountScript: Boolean): String =
    s"""
       | match tx {
       |  case ext : ExchangeTransaction =>
       |    # Neel context
       ...
       |
       |     let balances = assetBalance(ext.sender, unit) > 0 && neelBalance(ext.sender) != 0
       |
