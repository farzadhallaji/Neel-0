...
import com.neelnetwork.it.api.{AssetDecimalsInfo, LevelResponse}
...
import com.neelnetwork.transaction.assets.exchange.{AssetPair, _}
...
class MatcherTestSuite extends MatcherSuiteBase with TableDrivenPropertyChecks {
  ...
  "Check cross ordering between Alice and Bob" - {
    ...
    val order1         = matcherNode.prepareOrder(aliceAcc, aliceNeelPair, SELL, aliceSellAmount, 2000.neel, version = orderVersion, timeToLive = 2.minutes)
    val order1Response = matcherNode.placeOrder(order1)

    // Bob issues new asset
    val bobNeelPair = AssetPair(
    ...
    "get opened trading markets" in {
      ...
      markets.priceAssetName shouldBe "NEEL"
    ...
    "sell order could be placed correctly" - {
      "alice places sell order" in {
        ...
        matcherNode.waitOrderStatus(aliceNeelPair, order1Response.message.id, "Accepted")

        // Alice check that order is correct
        val orders = matcherNode.orderBook(aliceNeelPair)
        orders.asks.head.amount shouldBe aliceSellAmount
        orders.asks.head.price shouldBe 2000.neel
      }
      ...
      "and should match with buy order" in {
        ...
        val order2 = matcherNode.placeOrder(bobAcc, aliceNeelPair, BUY, 200, 2.neel * Order.PriceConstant, matcherFee, orderVersion)
        order2.status shouldBe "OrderAccepted"

        matcherNode.waitOrderStatus(aliceNeelPair, order1Response.message.id, "PartiallyFilled")
        matcherNode.waitOrderStatus(aliceNeelPair, order2.message.id, "Filled")

        matcherNode.orderHistoryByPair(bobAcc, aliceNeelPair).map(_.id) should contain(order2.message.id)
        ...
        // Bob checks that he spent some Neel
        ...
        // Alice checks that she received some Neel
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder = matcherNode.prepareOrder(bobAcc, aliceNeelPair, SELL, 300, 1900.neel, orderVersion)
        ...
        val order3 = matcherNode.placeOrder(bobAcc, aliceNeelPair, SELL, 150, 1900.neel, matcherFee, orderVersion)
        matcherNode.waitOrderStatus(aliceNeelPair, order3.message.id, "Accepted")

        // Bob checks that the order in the order book
        val orders = matcherNode.orderBook(aliceNeelPair)
        orders.asks should contain(LevelResponse(150, 1900.neel))
      }
      ...
      "buy order should match on few price levels" in {
        ...
        val order4 =
          matcherNode.placeOrder(aliceAcc, aliceNeelPair, BUY, 350, (21.neel / 10.0 * Order.PriceConstant).toLong, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")

        // Where were 2 sells that should fulfill placed order
        matcherNode.waitOrderStatus(aliceNeelPair, order4.message.id, "Filled")
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val status1 = matcherNode.cancelOrder(aliceAcc, aliceNeelPair, order1Response.message.id)
        status1.status should be("OrderCanceled")

        // Alice checks that the order book is empty
        val orders1 = matcherNode.orderBook(aliceNeelPair)
        orders1.asks.size should be(0)
        orders1.bids.size should be(0)

        // Alice places a new sell order on 100
        val order4 = matcherNode.placeOrder(aliceAcc, aliceNeelPair, SELL, 100, 2000.neel, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")

        // Alice checks that the order is in the order book
        val orders2 = matcherNode.orderBook(aliceNeelPair)
        orders2.asks should contain(LevelResponse(100, 2000.neel))
      }
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = matcherNode.placeOrder(bobAcc, aliceNeelPair, BUY, 130, 2000.neel, matcherFee, orderVersion)

        // Check that the order is partially filled
        matcherNode.waitOrderStatus(aliceNeelPair, order5.message.id, "PartiallyFilled")

        // Check that remaining part of the order is in the order book
        val orders = matcherNode.orderBook(aliceNeelPair)
        orders.bids should contain(LevelResponse(30, 2000.neel))
      ...
      "request order book for blacklisted pair" in {
        val f = matcherNode.matcherGetStatusCode(s"/matcher/orderbook/$ForbiddenAssetId/NEEL", 404)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        val bobNeelPair = AssetPair(ByteStr.decodeBase58(bobAsset).toOption, None)

        def bobOrder = matcherNode.prepareOrder(bobAcc, bobNeelPair, SELL, someAssetAmount, 0.005.neel, matcherFee, orderVersion)

        val order6 = matcherNode.placeOrder(bobOrder)
        matcherNode.waitOrderStatus(bobNeelPair, order6.message.id, "Accepted")

        // Alice wants to buy all Bob's assets for 1 Wave
        val order7 = matcherNode.placeOrder(aliceAcc, bobNeelPair, BUY, someAssetAmount, 0.005.neel, matcherFee, orderVersion)
        matcherNode.waitOrderStatus(bobNeelPair, order7.message.id, "Filled")
      ...
      "trader can buy neel for assets with order without having neel" in {
        val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
        ...
        def bobOrder =
          matcherNode.prepareOrder(bobAcc, bobNeelPair, SELL, someAssetAmount, 1.neel, matcherFee, orderVersion)

        val order8 = matcherNode.placeOrder(bobOrder)
        matcherNode.waitOrderStatus(bobNeelPair, order8.message.id, "Accepted")
        matcherNode.reservedBalance(bobAcc)
        // Bob moves all neel to Alice
        ...
        // Cleanup
        matcherNode.cancelOrder(bobAcc, bobNeelPair, order8.message.id).status should be("OrderCanceled")
      ...
      "market status" in {
        ...
        matcherNode.placeOrder(bobAcc, bobNeelPair, SELL, askAmount, ask, matcherFee, orderVersion)

        val resp1 = matcherNode.marketStatus(bobNeelPair)
        ...
        matcherNode.placeOrder(aliceAcc, bobNeelPair, BUY, bidAmount, bid, matcherFee, orderVersion)

        val resp2 = matcherNode.marketStatus(bobNeelPair)
