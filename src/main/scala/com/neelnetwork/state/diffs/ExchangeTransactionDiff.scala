...
import com.neelnetwork.transaction.ValidationError.{GenericError, OrderValidationError}
...
object ExchangeTransactionDiff {
  ...
  def apply(blockchain: Blockchain, height: Int)(tx: ExchangeTransaction): Either[ValidationError, Diff] = {
    ...
    for {
      ...
      def neelPortfolio(amt: Long) = Portfolio(amt, LeaseBalance.empty, Map.empty)

      val feeDiff = Monoid.combineAll(
        Seq(
          Map(matcher -> neelPortfolio(t.buyMatcherFee + t.sellMatcherFee - t.fee)),
          Map(buyer   -> neelPortfolio(-t.buyMatcherFee)),
          Map(seller  -> neelPortfolio(-t.sellMatcherFee))
        ))
