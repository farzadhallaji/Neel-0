...
import com.neelnetwork.account.Alias
import com.neelnetwork.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
import com.neelnetwork.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import com.neelnetwork.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.neel balance is decreased by fee.") {
  ...
  test("neel mass transfer changes neel balances") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough neel") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative neel balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.neel), Transfer(secondAddress, 2.neel), Transfer(thirdAddress, 3.neel))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.neel)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.neel)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.buildWithCurrentNetworkByte(alias).explicitGet().stringRepr, 2.neel)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
