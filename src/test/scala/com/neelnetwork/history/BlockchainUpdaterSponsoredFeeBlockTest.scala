...
import com.neelnetwork.transaction.assets.{IssueTransaction, SponsorFeeTransaction}
...
class BlockchainUpdaterSponsoredFeeBlockTest
  ...
  val sponsorPreconditions: Gen[Setup] = for {
    ...
    neelFee                    = Sponsorship.toNeel(sponsorTx.minSponsoredAssetFee.get, sponsorTx.minSponsoredAssetFee.get)
    genesis: GenesisTransaction = GenesisTransaction.create(master, ENOUGH_AMT, ts).explicitGet()
    masterToAlice: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(None,
                  master,
                  alice,
                  feeAsset.fee + sponsorTx.fee + transferAssetNeelFee + neelFee,
                  ts + 1,
                  None,
                  transferAssetNeelFee,
  ...
  val SponsoredActivatedAt0NeelSettings: NeelSettings = settings.copy(blockchainSettings = SponsoredFeeActivatedAt0BlockchainSettings)

  property("not enough neel to sponsor sponsored tx") {
    scenario(sponsorPreconditions, SponsoredActivatedAt0NeelSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, bobToMaster2)) =>
        ...
        domain.blockchainUpdater.processBlock(block4) should produce("negative neel balance" /*"unavailable funds"*/ )
