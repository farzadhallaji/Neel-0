...
import com.neelnetwork.db.{Storage, VersionedStorage}
import com.neelnetwork.lang.v1.compiler.CompilerContext
...
import com.neelnetwork.lang.v1.{CTX, FunctionHeader, ScriptEstimator}
...
package object utils extends ScorexLogging {
  ...
  def forceStopApplication(reason: ApplicationStopReason = Default): Unit =
    ...
    }, "neel-platform-shutdown-thread").start()
        ...
        version -> Coeval.evalOnce(
          Monoid
            .combineAll(Seq(
              ...
              NeelContext
                .build(version, new NeelEnvironment(AddressScheme.current.chainId, Coeval(???), Coeval(???), EmptyBlockchain), isTokenContext = true)
            )))
      }
      .toMap
        ...
        version -> Coeval.evalOnce(
          Monoid
            .combineAll(Seq(
              PureContext.build(version),
              CryptoContext.build(Global),
              NeelContext.build(version,
                                 new NeelEnvironment(AddressScheme.current.chainId, Coeval(???), Coeval(???), EmptyBlockchain),
