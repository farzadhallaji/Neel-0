...
import com.neelnetwork.account.{AddressOrAlias, AddressScheme, Alias}
import com.neelnetwork.database.LevelDBWriter
import com.neelnetwork.db.LevelDBFactory
import com.neelnetwork.lang.v1.traits.Environment
import com.neelnetwork.lang.v1.traits.domain.Recipient
import com.neelnetwork.settings.{NeelSettings, loadConfig}
import com.neelnetwork.state.NeelEnvironmentBenchmark._
import com.neelnetwork.state.bench.DataTestData
import com.neelnetwork.transaction.smart.NeelEnvironment
...
/**
  ...
  * 2. Import it: https://github.com/neelnetwork/Neel/wiki/Export-and-import-of-the-blockchain#import-blocks-from-the-binary-file
...
class NeelEnvironmentBenchmark {
  ...
  @Benchmark
  def accountBalanceOf_neel_test(st: AccountBalanceOfNeelSt, bh: Blackhole): Unit = {
...
object NeelEnvironmentBenchmark {
  ...
  @State(Scope.Benchmark)
  class AccountBalanceOfNeelSt extends BaseSt {
  ...
  @State(Scope.Benchmark)
  class AccountBalanceOfAssetSt extends AccountBalanceOfNeelSt {
  ...
  class BaseSt {
    ...
    private val neelSettings: NeelSettings = {
      val config = loadConfig(ConfigFactory.parseFile(new File(benchSettings.networkConfigFile)))
      NeelSettings.fromConfig(config)
    }

    AddressScheme.current = new AddressScheme {
      override val chainId: Byte = neelSettings.blockchainSettings.addressSchemeCharacter.toByte
    }

    private val db: DB = {
      val dir = new File(neelSettings.dataDirectory)
      if (!dir.isDirectory) throw new IllegalArgumentException(s"Can't find directory at '${neelSettings.dataDirectory}'")
      LevelDBFactory.factory.open(dir, new Options)
    }

    val environment: Environment = {
      val state = new LevelDBWriter(db, neelSettings.blockchainSettings.functionalitySettings, 100000, 2000, 120 * 60 * 1000)
      new NeelEnvironment(
