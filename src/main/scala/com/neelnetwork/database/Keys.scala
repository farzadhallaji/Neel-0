...
import com.neelnetwork.transaction.smart.script.{Script, ScriptReader}
...
object Keys {
  ...
  def neelBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey("neel-balance-history", 5, addressId.toByteArray)
  def neelBalance(addressId: BigInt)(height: Int): Key[Long] =
    Key("neel-balance", hAddr(6, height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  val addressesForNeelSeqNr: Key[Int]                = intKey("addresses-for-neel-seq-nr", 37)
  def addressesForNeel(seqNr: Int): Key[Seq[BigInt]] = Key("addresses-for-neel", h(38, seqNr), readBigIntSeq, writeBigIntSeq)
