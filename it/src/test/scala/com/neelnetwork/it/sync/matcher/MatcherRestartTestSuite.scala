  ...
  "check order execution" - {
    ...
    "make order and after matcher's restart try to cancel it" in {
      ...
      val aliceOrder = matcherNode
        .placeOrder(aliceAcc, aliceNeelPair, OrderType.SELL, 500, 2.neel * Order.PriceConstant, matcherFee, orderVersion)
      ...
      orders.asks.head.price shouldBe 2.neel * Order.PriceConstant
      ...
      orders1.asks.head.price shouldBe 2.neel * Order.PriceConstant

      val aliceSecondOrder =
        matcherNode.placeOrder(aliceAcc, aliceNeelPair, OrderType.SELL, 500, 2.neel * Order.PriceConstant, matcherFee, orderVersion, 5.minutes)
      ...
      orders2.asks.head.price shouldBe 2.neel * Order.PriceConstant

      val cancel = matcherNode.cancelOrder(aliceAcc, aliceNeelPair, firstOrder)
      cancel.status should be("OrderCanceled")

      val orders3 = matcherNode.orderBook(aliceNeelPair)
      orders3.asks.head.amount shouldBe 500

      matcherNode.waitOrderStatus(aliceNeelPair, firstOrder, "Cancelled")
      matcherNode.fullOrderHistory(aliceAcc).head.status shouldBe "Accepted"
    }
  }
}
