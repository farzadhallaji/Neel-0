package com.neelnetwork.lagonaki.unit
...
import com.neelnetwork.block.{Block, SignerData}
...
class BlockSpecification extends PropSpec with PropertyChecks with TransactionGen with Matchers with NoShrink {
  ...
  val blockGen = for {
    ...
    paymentTransaction        <- neelTransferGeneratorP(time, sender, recipient)
    transferTrancation        <- transferGeneratorP(1 + time, sender, recipient, assetId, None)
    anotherPaymentTransaction <- neelTransferGeneratorP(2 + time, sender, recipient)
  ...
  def bigBlockGen(amt: Int): Gen[Block] =
    for {
      ...
      sender                                    <- accountGen
      recipient                                 <- accountGen
      paymentTransaction: TransferTransactionV1 <- neelTransferGeneratorP(time, sender, recipient)
    } yield
      Block
        .buildAndSign(3,
                      time,
