...
class BlockchainUpdaterGeneratorFeeNextBlockOrMicroBlockTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    somePayment: TransferTransactionV1 = createNeelTransfer(sender, recipient, 1, 10, ts + 1).explicitGet()
    // generator has enough balance for this transaction if gets fee for block before applying it
    generatorPaymentOnFee: TransferTransactionV1 = createNeelTransfer(defaultSigner, recipient, 11, 1, ts + 2).explicitGet()
    someOtherPayment: TransferTransactionV1      = createNeelTransfer(sender, recipient, 1, 1, ts + 3).explicitGet()
  ...
  property("generator should get fees before applying block before applyMinerFeeWithTransactionAfter in two blocks") {
    ...
    scenario(preconditionsAndPayments, DefaultNeelSettings) {
