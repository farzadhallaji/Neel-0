...
import com.neelnetwork.transaction.ValidationError.{AliasDoesNotExist, AliasIsDisabled}
...
class CompositeBlockchain(inner: Blockchain, maybeDiff: => Option[Diff], carry: Long = 0) extends Blockchain {
  ...
  override def neelDistribution(height: Int): Map[Address, Long] = {
    val innerDistribution = inner.neelDistribution(height)
