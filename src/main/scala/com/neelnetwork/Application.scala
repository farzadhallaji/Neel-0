...
import com.neelnetwork.actor.RootActorSystem
import com.neelnetwork.api.http._
import com.neelnetwork.api.http.alias.{AliasApiRoute, AliasBroadcastApiRoute}
import com.neelnetwork.api.http.assets.{AssetsApiRoute, AssetsBroadcastApiRoute}
import com.neelnetwork.api.http.leasing.{LeaseApiRoute, LeaseBroadcastApiRoute}
import com.neelnetwork.consensus.PoSSelector
import com.neelnetwork.consensus.nxt.api.http.NxtConsensusApiRoute
import com.neelnetwork.db.openDB
import com.neelnetwork.features.api.ActivationApiRoute
import com.neelnetwork.history.{CheckpointServiceImpl, StorageFactory}
import com.neelnetwork.http.{DebugApiRoute, NodeApiRoute, NeelApiRoute}
import com.neelnetwork.matcher.Matcher
import com.neelnetwork.metrics.Metrics
import com.neelnetwork.mining.{Miner, MinerImpl}
import com.neelnetwork.network.RxExtensionLoader.RxExtensionLoaderShutdownHook
import com.neelnetwork.network._
import com.neelnetwork.settings._
import com.neelnetwork.state.appender.{BlockAppender, CheckpointAppender, ExtensionAppender, MicroblockAppender}
import com.neelnetwork.utils.{NTP, ScorexLogging, SystemInformationReporter, forceStopApplication}
import com.neelnetwork.utx.{MatcherUtxPool, UtxPool, UtxPoolImpl}
...
class Application(val actorSystem: ActorSystem, val settings: NeelSettings, configRoot: ConfigObject, time: NTP) extends ScorexLogging {
  ...
  def run(): Unit = {
    ...
    if (settings.restAPISettings.enable) {
      val apiRoutes = Seq(
        ...
        NeelApiRoute(settings.restAPISettings, wallet, utxStorage, allChannels, time),
...
object Application extends ScorexLogging {
    ...
    val config = maybeConfigFile match {
      ...
      case Some(file) =>
        ...
        if (!cfg.hasPath("neel")) {
          ...
          log.error("https://github.com/neelnetwork/Neel/wiki/Neel-Node-configuration-file")
  ...
  def main(args: Array[String]): Unit = {
    ...
    System.setProperty("neel.directory", config.getString("neel.directory"))
    log.info("Starting...")
    sys.addShutdownHook {
      SystemInformationReporter.report(config)
    }

    val settings = NeelSettings.fromConfig(config)
    ...
    RootActorSystem.start("neelnetwork", config) { actorSystem =>
