...
import com.neelnetwork.Shutdownable
import com.neelnetwork.settings.{Constants, RestAPISettings}
import com.neelnetwork.state.Blockchain
import io.swagger.annotations._
import javax.ws.rs.Path
import play.api.libs.json.Json
import com.neelnetwork.api.http.{ApiRoute, CommonApiFunctions}
...
case class NodeApiRoute(settings: RestAPISettings, blockchain: Blockchain, application: Shutdownable)
  ...
  @Path("/version")
  @ApiOperation(value = "Version", notes = "Get Neel node version", httpMethod = "GET")
  @ApiResponses(
    Array(
      new ApiResponse(code = 200, message = "Json Neel node version")
    ))
