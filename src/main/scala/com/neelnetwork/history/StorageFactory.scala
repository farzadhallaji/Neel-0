...
import com.neelnetwork.database.{DBExt, Keys, LevelDBWriter}
import com.neelnetwork.settings.NeelSettings
import com.neelnetwork.state.{BlockchainUpdaterImpl, NG}
import com.neelnetwork.transaction.BlockchainUpdater
...
object StorageFactory extends ScorexLogging {
  ...
  def apply(settings: NeelSettings, db: DB, time: Time): BlockchainUpdater with NG = {
    checkVersion(db)
