package com.neelnetwork.it.sync.smartcontract.smartasset

import com.neelnetwork.it.NTPTime
import com.neelnetwork.it.api.SyncHttpApi._
import com.neelnetwork.it.sync._
import com.neelnetwork.it.sync.smartcontract.{cryptoContextScript, pureContextScript, neelContextScript, _}
import com.neelnetwork.it.transactions.BaseTransactionSuite
import com.neelnetwork.state._
import com.neelnetwork.transaction.DataTransaction
...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("AssetPair from smart assets") {
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.neel") {
      ...
      assertBadRequestAndMessage(
        ...
        "com.neelnetwork.transaction.assets.exchange.ExchangeTransactionV2 does not exceed minimal value of 1500000"
      )
  ...
  test("use all functions from RIDE for asset script") {
    ...
    val script3 = Some(ScriptCompiler(neelContextScript(dtx, false), isAssetScript = true).explicitGet()._1.bytes.value.base64)
