...
import com.neelnetwork.it.api.AsyncHttpApi._
import com.neelnetwork.it.util.GlobalTimer.{instance => timer}
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false) extends AutoCloseable with ScorexLogging {
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("neel.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val neelNetwork: Network = {
    val networkName = s"neel-${hashCode().toLong.toHexString}"
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val restApiPort    = actualConfig.getString("neel.rest-api.port")
      val networkPort    = actualConfig.getString("neel.network.port")
      val matcherApiPort = actualConfig.getString("neel.matcher.port")
      ...
      val nodeName   = actualConfig.getString("neel.network.node-name")
      ...
      val configOverrides: String = {
        val ntpServer = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dneel.ntp-server=$x ")

        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dneel.network.declared-address=$ip:$networkPort $ntpServer"

        if (enableProfiling) {
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2018.04/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=NeelNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot "
        }
      ...
      val containerConfig = ContainerConfig
        .builder()
        .image("com.neelnetwork/it:latest")
        ...
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(
          neelNetwork.name() -> endpointConfigFor(nodeName)
        ).asJava))
        .hostConfig(hostConfig)
        .env(s"NEEL_OPTS=$configOverrides")
        .build()

      val containerId = {
        val containerName = s"${neelNetwork.name()}-$nodeName"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, NeelSettings.fromConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: NeelSettings): NodeInfo = {
    ...
    val neelIpAddress = containerInfo.networkSettings().networks().get(neelNetwork.name()).ipAddress()
    ...
    NodeInfo(
      ...
      new InetSocketAddress(neelIpAddress, networkPort)
    )
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(neelNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${neelNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/neel/start-neel.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$NEEL_OPTS.*-jar|$$NEEL_OPTS $renderedConfig -jar|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(neelNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${neelNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = client.disconnectFromNetwork(containerId, neelNetwork.id())
  ...
  private def updateStartScript(node: DockerNode): Unit = {
    ...
    log.debug("Make backup copy of /opt/neel/start-neel.sh")
    val cpCmd: Array[String] =
      Array(
        "sh",
        "-c",
        s"""cp /opt/neel/start-neel.sh /opt/neel/start-neel.sh.bk"""
      )
    ...
    val scriptCmd: Array[String] =
      Array(
        "sh",
        "-c",
        s"""rm /opt/neel/start-neel.sh && echo '#!/bin/bash' >> /opt/neel/start-neel.sh &&
             |echo 'java ${renderProperties(asProperties(genesisOverride))} -cp /opt/neel/neel.jar com.neelnetwork.matcher.MatcherTool /opt/neel/template.conf cb > /opt/neel/migration-tool.log' >> /opt/neel/start-neel.sh &&
             |echo 'less /opt/neel/migration-tool.log | grep -ir completed && cp /opt/neel/start-neel.sh.bk /opt/neel/start-neel.sh && chmod +x /opt/neel/start-neel.sh' >> /opt/neel/start-neel.sh &&
             |chmod +x /opt/neel/start-neel.sh
           """.stripMargin
      )
...
object Docker {
  private val ContainerRoot      = Paths.get("/opt/neel")
  ...
  def genesisOverride = {
    ...
    val timestampOverrides = parseString(s"""neel.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
                                            |  block-timestamp = $genesisTs
                                            |}""".stripMargin)

    val genesisConfig    = configTemplate.withFallback(timestampOverrides)
    val gs               = genesisConfig.as[GenesisSettings]("neel.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().uniqueId

    timestampOverrides.withFallback(parseString(s"neel.blockchain.custom.genesis.signature = $genesisSignature"))
  }

  AddressScheme.current = new AddressScheme {
    override val chainId = configTemplate.as[String]("neel.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
