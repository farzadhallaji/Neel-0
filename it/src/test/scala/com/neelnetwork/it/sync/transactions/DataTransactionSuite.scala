...
import com.neelnetwork.it.api.UnexpectedStatusCodeException
import com.neelnetwork.it.sync.{calcDataFee, minFee}
import com.neelnetwork.it.transactions.BaseTransactionSuite
import com.neelnetwork.it.util._
import com.neelnetwork.state.{BinaryDataEntry, BooleanDataEntry, ByteStr, DataEntry, EitherExt2, IntegerDataEntry, StringDataEntry}
...
class DataTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's neel balance is decreased by fee.") {
  ...
  test("cannot transact without having enough neel") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)

    val data = List(BooleanDataEntry("bool", false))
    assertBadRequestAndResponse(sender.putData(firstAddress, data, balance1 + 1), "negative neel balance")
    nodes.waitForHeightArise()
    notMiner.assertBalances(firstAddress, balance1, eff1)

    val leaseAmount = 1.neel
