...
import com.neelnetwork.matcher.model.{BuyLimitOrder, SellLimitOrder}
...
import com.neelnetwork.{NTPTime, crypto}
...
trait MatcherTestData extends NTPTime { _: Suite =>
  ...
  val neelAssetGen: Gen[Option[Array[Byte]]] = Gen.const(None)
  ...
  val maxNeelAmountGen: Gen[Long] = Gen.choose(1, 100000000L * 100000000L)
  ...
  def buyGenerator(pair: AssetPair,
    ...
    for {
      ...
      matcherFee: Long          <- matcherFee.map(Gen.const).getOrElse(maxNeelAmountGen)
  ...
  val orderGenerator: Gen[(Order, PrivateKeyAccount)] = for {
    ...
    amount: Long              <- maxNeelAmountGen
    price: Long               <- maxNeelAmountGen
    timestamp: Long           <- createdTimeGen
    expiration: Long          <- maxTimeGen
    matcherFee: Long          <- maxNeelAmountGen
