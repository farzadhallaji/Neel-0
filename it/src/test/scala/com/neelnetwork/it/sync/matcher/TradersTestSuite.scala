...
import com.neelnetwork.matcher.market.MatcherActor
import com.neelnetwork.matcher.model.MatcherModel.Price
...
class TradersTestSuite extends MatcherSuiteBase {
  ...
  "Verifications of tricky ordering cases" ignore {
    ...
    "owner moves assets/neel to another account and order become an invalid" ignore {
      ...
      "order with assets" - {
        "moved assets, insufficient assets" in {
          ...
          withClue(s"The oldest order '$oldestOrderId' was cancelled") {
            matcherNode.waitOrderStatus(bobNeelPair, oldestOrderId, "Cancelled")
          }
          withClue(s"The newest order '$newestOrderId' is still active") {
            matcherNode.waitOrderStatus(bobNeelPair, newestOrderId, "Accepted")
          }
        ...
        "leased neel, insufficient fee" in {
        ...
        "moved neel, insufficient fee" in {
          val bobBalance    = matcherNode.accountBalances(bobAcc.address)._1
          val oldestOrderId = bobPlacesAssetOrder(1000, twoAssetsPair, bobNewAsset)
          val newestOrderId = bobPlacesAssetOrder(1000, twoAssetsPair, bobNewAsset)
      ...
      "order with neel" - {
        "leased neel, insufficient fee for one ExchangeTransaction" in {
          // Amount of neel in order is smaller than fee
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1

          val oldestOrderId = bobPlacesWaveOrder(bobNeelPair, 1, 10.neel * Order.PriceConstant)
          val newestOrderId = bobPlacesWaveOrder(bobNeelPair, 1, 10.neel * Order.PriceConstant)

          //      waitForOrderStatus(matcherNode, bobAssetIdRaw, id, "Accepted")
          val leaseAmount = bobBalance - exTxFee - 10.neel - matcherFee
          val leaseId     = bobNode.lease(bobAcc.address, aliceAcc.address, leaseAmount, exTxFee, 2).id
          matcherNode.waitForTransaction(leaseId)
          ...
          // Cleanup
          matcherNode.cancelOrder(bobAcc, bobNeelPair, newestOrderId)
          matcherNode.waitOrderStatus(twoAssetsPair, newestOrderId, "Cancelled")
        ...
        "leased neel, insufficient neel" in {
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
          val price      = 1.neel
          val order2     = bobPlacesWaveOrder(bobNeelPair, 1, price * Order.PriceConstant)
          ...
          withClue(s"The order '$order2' was cancelled") {
            matcherNode.waitOrderStatus(bobNeelPair, order2, "Cancelled")
          }
        ...
        "moved neel, insufficient fee" in {
          ...
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
          val price      = exTxFee / 2
          val order3     = bobPlacesWaveOrder(bobNeelPair, 1, price * Order.PriceConstant)
          ...
          withClue(s"The order '$order3' was cancelled") {
            matcherNode.waitOrderStatus(bobNeelPair, order3, "Cancelled")
          }
