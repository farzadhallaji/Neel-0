...
import com.neelnetwork.settings.{TestFunctionalitySettings, NeelSettings, loadConfig}
import com.neelnetwork.state.diffs.ENOUGH_AMT
import com.neelnetwork.state.{BlockchainUpdaterImpl, EitherExt2}
import com.neelnetwork.transaction.smart.SetScriptTransaction
import com.neelnetwork.transaction.smart.script.v1.ScriptV1
import com.neelnetwork.transaction.transfer.{TransferTransaction, TransferTransactionV1}
import com.neelnetwork.transaction.{GenesisTransaction, Transaction}
import com.neelnetwork.utils.Time
import com.neelnetwork.{RequestGen, WithDB}
...
class LevelDBWriterSpec extends FreeSpec with Matchers with WithDB with RequestGen {
  ...
  def baseTest(gen: Time => Gen[(PrivateKeyAccount, Seq[Block])])(f: (LevelDBWriter, PrivateKeyAccount) => Unit): Unit = {
    ...
    val settings0     = NeelSettings.fromConfig(loadConfig(ConfigFactory.load()))
