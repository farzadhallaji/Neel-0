...
inTask(docker)(
  Seq(
    dockerfile := {
      ...
      val startNeel     = sourceDirectory.value / "container" / "start-neel.sh"
      ...
      new Dockerfile {
        ...
        if (withAspectJ) run("wget", "--quiet", aspectjAgentUrl, "-O", "/opt/neel/aspectjweaver.jar")

        add((assembly in LocalProject("node")).value, "/opt/neel/neel.jar")
        add(Seq(configTemplate, startNeel), "/opt/neel/")
        runShell("chmod", "+x", "/opt/neel/start-neel.sh")
        entryPoint("/opt/neel/start-neel.sh")
        expose(10001)
      }
    },
        ...
        Tests.SubProcess(
          ForkOptions(
            ...
            runJVMOptions = Vector(
              ...
              "-Dneel.it.logging.appender=FILE",
              s"-Dneel.it.logging.dir=${logDirectoryValue / suite.name.replaceAll("""(\w)\w*\.""", "$1.")}"
            ) ++ javaOptionsValue,
            connectInput = false,
