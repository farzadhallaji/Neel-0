...
import com.neelnetwork.it.sync.matcherFee
...
class SeveralPartialOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Alice and Bob trade NEEL-USD" - {
    ...
    "place usd-neel order" in {
      ...
      val aliceOrder2   = matcherNode.prepareOrder(aliceAcc, neelUsdPair, OrderType.BUY, buyOrderAmount, price)
      val aliceOrder2Id = matcherNode.placeOrder(aliceOrder2).message.id
      matcherNode.waitOrderStatus(neelUsdPair, aliceOrder2Id, "Filled", 1.minute)

      // Bob wants to buy some USD
      matcherNode.waitOrderStatus(neelUsdPair, bobOrder1Id, "Filled", 1.minute)
      ...
      val orderBook1 = matcherNode.orderBook(neelUsdPair)
      orderBook1.asks shouldBe empty
      orderBook1.bids shouldBe empty

      val bobOrder2   = matcherNode.prepareOrder(bobAcc, neelUsdPair, OrderType.SELL, sellOrderAmount, price)
      val bobOrder2Id = matcherNode.placeOrder(bobOrder2).message.id
      matcherNode.waitOrderStatus(neelUsdPair, bobOrder2Id, "Accepted", 1.minute)

      val orderBook2 = matcherNode.orderBook(neelUsdPair)
      orderBook2.asks shouldBe List(LevelResponse(bobOrder2.amount, bobOrder2.price))
      orderBook2.bids shouldBe empty

      matcherNode.cancelOrder(bobAcc, neelUsdPair, bobOrder2Id)
      matcherNode.waitOrderStatus(neelUsdPair, bobOrder2Id, "Cancelled", 1.minute)
    ...
    "place one submitted orders and two counter" in {
      val aliceOrder1   = matcherNode.prepareOrder(aliceAcc, neelUsdPair, OrderType.BUY, buyOrderAmount, price)
      ...
      matcherNode.waitOrderStatus(neelUsdPair, aliceOrder1Id, "Filled", 1.minute)
      matcherNode.waitOrderStatus(neelUsdPair, aliceOrder2Id, "Filled", 1.minute)
      matcherNode.waitOrderStatus(neelUsdPair, bobOrder1Id, "Filled", 1.minute)
