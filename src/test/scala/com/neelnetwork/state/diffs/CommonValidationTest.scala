...
import com.neelnetwork.transaction.assets.{IssueTransactionV1, IssueTransactionV2, SponsorFeeTransaction}
...
import com.neelnetwork.transaction.{GenesisTransaction, Transaction, ValidationError}
...
class CommonValidationTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with WithState with NoShrink {
  ...
  property("disallows double spending") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, TransferTransactionV1)] = for {
      ...
      transfer: TransferTransactionV1 <- neelTransferGeneratorP(master, recipient)
  ...
  private def sponsorAndSetScriptGen(sponsorship: Boolean, smartToken: Boolean, smartAccount: Boolean, feeInAssets: Boolean, feeAmount: Long) =
    for {
      ...
      val transferNeelTx = TransferTransactionV1
      ...
      (TestBlock.create(Vector[Transaction](genesisTx, issueTx, transferNeelTx, transferAssetTx) ++ sponsorTx ++ setScriptTx), transferBackTx)
    }
