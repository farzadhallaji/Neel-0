...
import com.neelnetwork.crypto.SignatureLength
import com.neelnetwork.db.WithDomain
import com.neelnetwork.features.BlockchainFeatures._
import com.neelnetwork.features._
import com.neelnetwork.lagonaki.mocks.TestBlock
import com.neelnetwork.lang.v1.compiler.Terms.TRUE
import com.neelnetwork.settings.{TestFunctionalitySettings, NeelSettings}
import com.neelnetwork.state.reader.LeaseDetails
import com.neelnetwork.transaction.ValidationError.AliasDoesNotExist
import com.neelnetwork.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
import com.neelnetwork.transaction.lease.{LeaseCancelTransactionV1, LeaseTransactionV1}
...
import com.neelnetwork.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.neelnetwork.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.neelnetwork.state.diffs.ENOUGH_AMT))
    ...
    "neel balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): NeelSettings = {
      ...
      history.DefaultNeelSettings.copy(blockchainSettings = history.DefaultNeelSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.neelnetwork.state.diffs.ENOUGH_AMT))
