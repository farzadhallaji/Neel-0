...
import com.neelnetwork.it.sync.{script, someAssetAmount, _}
...
import com.neelnetwork.transaction.assets.SetAssetScriptTransaction
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough neel") {
    val (balance, eff) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative neel balance")
