...
import com.neelnetwork.history
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithDomain {
  ...
  private val NeelSettings = history.DefaultNeelSettings.copy(
    blockchainSettings = history.DefaultNeelSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultNeelSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultNeelSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val NeelSettingsWithDoubling = NeelSettings.copy(
    blockchainSettings = NeelSettings.blockchainSettings.copy(
      functionalitySettings = NeelSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(NeelSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(NeelSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(NeelSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(NeelSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
