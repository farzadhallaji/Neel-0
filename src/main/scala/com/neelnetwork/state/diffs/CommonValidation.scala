...
import com.neelnetwork.transaction.ValidationError._
...
object CommonValidation {
  ...
  def disallowSendingGreaterThanBalance[T <: Transaction](blockchain: Blockchain,
    ...
    if (blockTime >= settings.allowTemporaryNegativeUntil) {
      def checkTransfer(sender: Address, assetId: Option[AssetId], amount: Long, feeAssetId: Option[AssetId], feeAmount: Long) = {
        ...
        val oldNeelBalance = blockchain.portfolio(sender).balance

        val newNeelBalance = oldNeelBalance + spendings.balance
        if (newNeelBalance < 0) {
          Left(
            GenericError(
              "Attempt to transfer unavailable funds: Transaction application leads to " +
                s"negative neel balance to (at least) temporary negative state, current balance equals $oldNeelBalance, " +
                s"spends equals ${spendings.balance}, result is $newNeelBalance"))
  ...
  def getMinFee(blockchain: Blockchain,
        ...
        for {
          ...
          r <- txAsset match {
            ...
            case Some(assetId) =>
              ...
              } yield (Some((assetId, assetInfo)), neelFee)
          }
        } yield r
    ...
    feeAfterSponsorship(tx.assetFee._1)
      ...
      .map {
        case (Some((assetId, assetInfo)), amountInNeel) =>
          (Some(assetId), Sponsorship.fromNeel(amountInNeel, assetInfo.sponsorship), amountInNeel)
        case (None, amountInNeel) => (None, amountInNeel, amountInNeel)
      }
  }
    ...
    if (height >= Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) {
      for {
        ...
        _ <- Either.cond(
          ...
          (),
          GenericError(
            s"Fee in ${feeAssetId.fold("NEEL")(_.toString)} for ${tx.builder.classTag} does not exceed minimal value of $minNeel NEEL: ${tx.assetFee._2}")
        )
      } yield ()
    } else {
