...
import com.neelnetwork.mining._
import com.neelnetwork.settings._
import com.neelnetwork.state.diffs._
import com.neelnetwork.state.{ByteStr, EitherExt2, _}
import com.neelnetwork.transaction.ValidationError.SenderIsBlacklisted
...
class UtxPoolSpecification extends FreeSpec with Matchers with MockFactory with PropertyChecks with TransactionGen with NoShrink with WithDB {
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = NeelSettings.fromConfig(config)
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createNeelTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: PrivateKeyAccount, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(None, sender, recipient, neel(1), ts, None, feeAmount, Array.emptyByteArray).explicitGet()
  }
  ...
  private def transactionV2Gen(sender: PrivateKeyAccount, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
    TransferTransactionV2.selfSigned(2, None, sender, recipient, neel(1), ts, None, feeAmount, Array.emptyByteArray).explicitGet()
  }
