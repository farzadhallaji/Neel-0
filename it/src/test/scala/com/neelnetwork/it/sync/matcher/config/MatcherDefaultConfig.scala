...
import com.neelnetwork.api.http.assets.SignedIssueV1Request
...
object MatcherDefaultConfig {
  ...
  val minerEnabled  = parseString(s"""
       |neel.miner.enable = yes
       |neel.miner.quorum = 0""".stripMargin)
  val minerDisabled = parseString("neel.miner.enable = no")
  val matcherConfig = parseString(s"""
                                     |neel.miner.enable = no
                                     |neel.matcher {
                                     |  enable = yes
