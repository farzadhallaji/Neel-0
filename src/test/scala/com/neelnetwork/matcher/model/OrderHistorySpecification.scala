...
import com.neelnetwork.WithDB
import com.neelnetwork.account.{Address, PrivateKeyAccount, PublicKeyAccount}
import com.neelnetwork.matcher.api.DBUtils
import com.neelnetwork.matcher.model.Events.{OrderAdded, OrderCanceled, OrderExecuted}
import com.neelnetwork.matcher.model.OrderHistorySpecification._
import com.neelnetwork.matcher.{MatcherKeys, MatcherTestData}
import com.neelnetwork.state.ByteStr
import com.neelnetwork.transaction.AssetId
import com.neelnetwork.transaction.assets.exchange.{AssetPair, Order}
...
class OrderHistorySpecification
  ...
  property("New buy NEEL order added") {
    ...
    withClue("reserved assets considering amount of received NEEL") {
  ...
  property("New sell NEEL order added") {
    val pair = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Should not reserve fee, if seller receives more NEEL than total fee in sell order") {
  ...
  property("Should not reserve fee, if buyer receives more NEEL than total fee in buy order") {
    val pair = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Buy NEEL order filled exactly") {
    val pair      = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Buy NEEL order filled with remainder") {
    val pair      = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Sell NEEL order - filled, buy order - partial") {
    val pair      = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Buy NEEL order - filled with 2 steps, sell order - partial") {
    val pair       = AssetPair(None, mkAssetId("BTC"))
  ...
  property("Sell ETH twice (filled, partial), buy NEEL order - filled") {
  ...
  property("Delete order") {
    ...
    openVolume(pk, pair.amountAsset) shouldBe 0 // We receive 210000000 >> 300000 NEEL
